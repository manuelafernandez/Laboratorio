...
class TCPDF {
	...
	//variable asignatura y nroGuia

	protected $piepagina2;

	protected $piepagina1;
	/**
	 * Current page number.
	 * @protected
	 */
	 ...
	 */
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false, $piepagina1="-", $piepagina2="X") {
		...
		//inicializar asignatura y nroGuia
	...
	public function AddPage($orientation='', $format='', $keepmargins=false, $tocpage=false) {
		...
		if (!isset($this->original_lMargin) OR $keepmargins) {
			$this->original_lMargin = 28;
		}
		if (!isset($this->original_rMargin) OR $keepmargins) {
			$this->original_rMargin = 30;
		}
	...
	public function Footer() {
		...
		$this->Cell(0, 15, $pagenumtxt, 0, 0, 'RP');
		
		$this->SetX($this->original_lMargin);
		$this->Cell(0, 7, $this->piepagina1, 'T', 0, 'L');

		$this->SetX($this->original_lMargin);
		$this->Cell(0, 15, $this->piepagina2, 'T', 1, 'L');
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	public function addTOC($page='', $numbersfont='', $filler='.', $toc_name='TOC', $style='', $color=array(0,0,0)) {
		...
		foreach ($this->outlines as $key => $outline) {
			...
			if ($numfills > 0) {
				$rowfill = str_repeat($filler, $numfills-50);
			} else {
				$rowfill = '';
			}
			if ($this->rtl){
			...
			// write the number
			$tw = $tw-3;
			$this->SetFont($numbersfont, 'B', $fontsize);
			$this->writeHTMLCell(153, 0, '', '', $txt, 0, 1, 0, true, '', true);
			$this->y = $this->y - 4.75;
